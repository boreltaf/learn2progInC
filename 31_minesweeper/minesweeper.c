...
int countMines(board_t * b, int x, int y) {
  int count=0;
  if(x-1>=0){
    if((IS_MINE(b->board[y][x-1]))||(IS_MINE(b->board[y][x-1]))){
      count++;
    }
  }
  if((x-1>=0)&&(y-1>=0)){
    if((IS_MINE(b->board[y-1][x-1]))||(IS_MINE(b->board[y-1][x-1]))){
      count++;
    }
  }
  if((x-1>=0)&&((y+1)<b->height)){
    if((IS_MINE(b->board[y+1][x-1]))||(IS_MINE(b->board[y+1][x-1]))){
      count++;
    }
  }
  if(y-1>=0){
    if((IS_MINE(b->board[y-1][x]))||(IS_MINE(b->board[y-1][x]))){
      count++;
    }
  }
  if((y-1>=0)&&((x+1)<b->width)){
    if((IS_MINE(b->board[y-1][x+1]))||(IS_MINE(b->board[y-1][x+1]))){
	count++;
     }
   }
